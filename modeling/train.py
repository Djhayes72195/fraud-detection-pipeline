...
def make_model_id(hyperparams, train_ids_hash):
    ...
    to_hash = {"hyperparameters": hyperparams, "train_ids_hash": train_ids_hash}
...
def train(df):
    ...
    model_id = write_metadata(train_ids, hyperparams, baseline_metrics)
...
def write_metadata(train_ids, hyperparams, baseline_metrics):
    """
    Write metadata related to the training process.

    This function writes JSON files containing:
        - Training IDs used.
        - Model metadata.
        - Performance metrics (accuracy, recall, etc...)

    Files are identified using generated hashes.
    ...
    """
    train_ids_hash = hashlib.md5(
        json.dumps(train_ids, sort_keys=True).encode()
    ).hexdigest()
    ...
    metrics_file = PERFORMANCE_METRICS_PATH / f"metrics_{model_id}.json"
...
def _persistent_data_split(X, y):
    ...
    return (X_train, X_val, X_test, y_train["Class"], y_val["Class"], y_test["Class"])
